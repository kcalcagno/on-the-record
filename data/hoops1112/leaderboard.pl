...
$sth = $dbh->prepare("select * from Games where BigEast = 1");
$sth->execute;
...
sub gameBoard {
    ...
    while ($row = $sth->fetchrow_hashref) {
        print GAMEFILE $html->tr_start();
        print GAMEFILE $html->td({'class' => "scores"}, ++$count . ".");
...
sub leaderBoard {
    ...
    if ($bigeast) {
        print OUTFILE $html->p("The worst score for each player is thrown out. Players who miss four or more games are removed.");
    } else {
        print OUTFILE $html->p("The worst two scores for each player are thrown out. Players who miss eight or more games are removed.");
    }

    foreach $key (sort {$a <=> $b} keys %$tref) {
        $row = $tref->{$key};
        if ($$row{"NDScore"}) {
            my $gamefile = sprintf("game%02d.html", $key);
            push @headers, $html->a({'href' => $gamefile}, $$row{"ShortName"});
        }
    }
    ...
    print OUTFILE $html->col_mini({'class' => "scores"});
    ...
    foreach $record (@board) {
        print OUTFILE $html->tr_start();
        print OUTFILE $html->td({'class' => "scores"}, ++$count . ".");
